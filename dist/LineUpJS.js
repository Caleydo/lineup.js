...
/***/ function(module, exports, __webpack_require__) {
	...
	var model_ = __webpack_require__(4);
	var provider_ = __webpack_require__(7);
	var renderer_ = __webpack_require__(8);
	var ui_ = __webpack_require__(9);
	var utils_ = __webpack_require__(6);
	var ui_dialogs_ = __webpack_require__(10);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var HeatmapCellRenderer = (function (_super) {
	    ...
	    HeatmapCellRenderer.prototype.render = function ($col, col, rows, context) {
	        ...
	        var $rects = $rows_enter.selectAll('rect').data(function (d) { return col.getValue(d); });
	...
	var VertcontinuousCellRenderer = (function (_super) {
	    __extends(VertcontinuousCellRenderer, _super);
	    function VertcontinuousCellRenderer() {
	        _super.apply(this, arguments);
	    }
	    VertcontinuousCellRenderer.prototype.render = function ($col, col, rows, context) {
	        ...
	        $rects.attr({
	            'data-index': function (d, i) { return i; },
	            'width': celldimension,
	            'height': function (d) { return (d < threshold) ? (barheight / 2 - scale(d)) : scale(d); },
	            'x': function (d, i) { return (i === null || 0) ? 0 : (i * celldimension); },
	            'y': function (d, i) {
	                if (min < 0) {
	    ...
	    return VertcontinuousCellRenderer;
	}(DefaultCellRenderer));
	var BoxplotCellRenderer = (function (_super) {
	    ...
	    BoxplotCellRenderer.prototype.render = function ($col, col, rows, context) {
	        ...
	        $rows_enter.append('rect').attr('class', 'boxplotrect');
	        $rows.select('rect.boxplotrect').attr({
	            'data-index': function (d, i) { return i; },
	            'width': function (d, i) { return scale(q3arr[i]) - scale(q1arr[i]); },
	            'height': function (d, i) {
	                var top = context.option('rowPadding', 30);
	                return (Math.max(context.rowHeight(i) - top, 10));
	            },
	            'y': 0,
	            'x': function (d, i) { return scale(q1arr[i]); }
	        });
	        $rows_enter.append('path').attr('class', 'boxplotline');
	        $rows.select('path.boxplotline').attr('d', function (d, i) {
	            ...
	            return 'M' + left + ',' + middle + 'L' + scale(q1arr[i]) + ',' + middle +
	                ...
	                'M' + (scale(q1arr[i]) + scale(q3arr[i]) - scale(q1arr[i])) + ',' + middle + 'L' + (right) + ',' + middle +
	...
	var CategoricalCellRenderer = (function (_super) {
	    __extends(CategoricalCellRenderer, _super);
	    function CategoricalCellRenderer() {
	        _super.apply(this, arguments);
	    }
	    CategoricalCellRenderer.prototype.render = function ($col, col, rows, context) {
	        ...
	        $circle.enter().append('circle')
	            ...
	            .attr('cx', function (d, i) { return (i * windowsize) + (windowsize / 2); })
	            .attr('cy', function (d, i) { return (context.rowHeight(i) / 2); })
	            .attr('r', (windowsize / 4))
	            .attr('class', 'categoricalcircle')
	            .attr('opacity', function (d) { return (d === 1) ? 1 : 0.1; });
	        $rows_enter.append('path')
	            .filter(function (d) {
	            var value = col.getValue(d);
	            var countcategory = value.filter(function (x) { return x === 1; }).length;
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.sortdialogForm = sortdialogForm;
	/**
	...
	exports.makesortPopup = makesortPopup;
	/**
