...
abstract class ADialog {
  ...
  constructor(readonly attachment: Selection<any, any, any, any>, private readonly title: string) {}
  ...
  makePopup(body: string) {
      ...
      function movePopup() {
        ...
        $popup.style('left',(this.parentElement.offsetLeft + (<any>d3event).dx) + 'px')
          .style('top', (this.parentElement.offsetTop + (<any>d3event).dy) + 'px');
      }

      $popup.select('span.lu-popup-title').call(drag().on('drag', movePopup));
      $popup.on('keydown', () => {
  ...
  hidePopupOnClickOutside(popup: Selection<any, any, any, any>, rendererContent: Selection<any, any, any, any>) {
